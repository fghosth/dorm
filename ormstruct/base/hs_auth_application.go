...
func getHsAuthApplicationArgsStrUpdate() string {
	...
	switch driverHsAuthApplication {
	case "mysql":
		argsStr = "secret_key=?,app_key=?,name=?,ip=?,type=?,exp=?,created_at=?,updated_at=?,deleted_at=?," + SDELFLAG + "=? WHERE " + SDELFLAG + "=0 and id=?"
	case "mariadb":
		argsStr = "secret_key=?,app_key=?,name=?,ip=?,type=?,exp=?,created_at=?,updated_at=?,deleted_at=?," + SDELFLAG + "=? WHERE " + SDELFLAG + "=0 and  id=?"
	case "cockroachDB":
		argsStr = "secret_key=$1,app_key=$2,name=$3,ip=$4,type=$5,exp=$6,created_at=$7,updated_at=$8,deleted_at=$9," + SDELFLAG + "=$10 WHERE " + SDELFLAG + "=0 and  id=$11"
	case "postgresql":
		argsStr = "secret_key=$1,app_key=$2,name=$3,ip=$4,type=$5,exp=$6,created_at=$7,updated_at=$8,deleted_at=$9," + SDELFLAG + "=$10 WHERE " + SDELFLAG + "=0 and  id=$11"
	}
	return argsStr
...
func (hsAuthApplication HsAuthApplication) Select(sql string, limit, offset int, value ...interface{}) ([]interface{}, error) {
	...
	// ar[0].(*HsAuthRecords)
	sqlstr := "select id,secret_key,app_key,name,ip,type,exp,created_at,updated_at,deleted_at," + SDELFLAG + " from hs_auth_application where " + SDELFLAG + " = 0 " + sql + " limit " + strconv.Itoa(limit) + " offset " + strconv.Itoa(offset)
...
func (hsAuthApplication *HsAuthApplication) FindByID(id int64) (interface{}, error) {
	...
	args[0] = id
	sqlstr := "SELECT id,secret_key,app_key,name,ip,type,exp,created_at,updated_at,deleted_at," + SDELFLAG + " FROM hs_auth_application WHERE " + SDELFLAG + "=0 and id = " + argsStr
	sqlHsAuthApplication = sqlstr
...
func (hsAuthApplication HsAuthApplication) Add() (int64, error) {
	...
	sqlstr := "INSERT INTO hs_auth_application (secret_key,app_key,name,ip,type,exp,created_at,updated_at,deleted_at," + SDELFLAG + ") VALUES (" + argsStr + ")"
